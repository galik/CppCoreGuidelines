...
* [GSL.view: Views](20-GSL-Guidelines%20support%20library.md#SS-views)
* [GSL.owner](20-GSL-Guidelines%20support%20library.md#SS-ownership)
* [GSL.assert: Assertions](20-GSL-Guidelines%20support%20library.md#SS-assertions)
* [GSL.util: Utilities](20-GSL-Guidelines%20support%20library.md#SS-utilities)
* [GSL.concept: Concepts](20-GSL-Guidelines%20support%20library.md#SS-gsl-concepts)
...
References are never owners (see [R.4](06-R-Resource%20management.md#Rr-ref). Note: References have many opportunities to outlive the objects they refer to (returning a local variable by reference, holding a reference to an element of a vector and doing `push_back`, binding to `std::max(x, y + 1)`, etc. The Lifetime safety profile aims to address those things, but even so `owner<T&>` does not make sense and is discouraged.
...
* `Number`   // ???
* `Sortable`
* `Pointer`  // A type with `*`, `->`, `==`, and default construction (default construction is assumed to set the singular "null" value); see [smart pointers](20-GSL-Guidelines%20support%20library.md#SS-gsl-smartptrconcepts)
* `Unique_ptr`  // A type that matches `Pointer`, has move (not copy), and matches the Lifetime profile criteria for a `unique` owner type; see [smart pointers](20-GSL-Guidelines%20support%20library.md#SS-gsl-smartptrconcepts)
* `Shared_ptr`   // A type that matches `Pointer`, has copy, and matches the Lifetime profile criteria for a `shared` owner type; see [smart pointers](20-GSL-Guidelines%20support%20library.md#SS-gsl-smartptrconcepts)
