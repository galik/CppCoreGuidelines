...
* [F.1: "Package" meaningful operations as carefully named functions](S-05-Functions.md#Rf-package)
* [F.2: A function should perform a single logical operation](S-05-Functions.md#Rf-logical)
* [F.3: Keep functions short and simple](S-05-Functions.md#Rf-single)
* [F.4: If a function may have to be evaluated at compile time, declare it `constexpr`](S-05-Functions.md#Rf-constexpr)
* [F.5: If a function is very small and time-critical, declare it inline](S-05-Functions.md#Rf-inline)
* [F.6: If your function may not throw, declare it `noexcept`](S-05-Functions.md#Rf-noexcept)
* [F.7: For general use, take `T*` or `T&` arguments rather than smart pointers](S-05-Functions.md#Rf-smart)
* [F.8: Prefer pure functions](S-05-Functions.md#Rf-pure)
* [F.9: Unused parameters should be unnamed](S-05-Functions.md#Rf-unused)

Parameter passing expression rules:

* [F.15: Prefer simple and conventional ways of passing information](S-05-Functions.md#Rf-conventional)
* [F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const`](S-05-Functions.md#Rf-in)
* [F.17: For "in-out" parameters, pass by reference to non-`const`](S-05-Functions.md#Rf-inout)
* [F.18: For "consume" parameters, pass by `X&&` and `std::move` the parameter](S-05-Functions.md#Rf-consume)
* [F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter](S-05-Functions.md#Rf-forward)
* [F.20: For "out" output values, prefer return values to output parameters](S-05-Functions.md#Rf-out)
* [F.21: To return multiple "out" values, prefer returning a tuple or struct](S-05-Functions.md#Rf-out-multi)
* [F.60: Prefer `T*` over `T&` when "no argument" is a valid option](S-05-Functions.md#Rf-ptr-ref)

Parameter passing semantic rules:

* [F.22: Use `T*` or `owner<T*>` or a smart pointer to designate a single object](S-05-Functions.md#Rf-ptr)
* [F.23: Use a `not_null<T>` to indicate "null" is not a valid value](S-05-Functions.md#Rf-nullptr)
* [F.24: Use a `span<T>` or a `span_p<T>` to designate a half-open sequence](S-05-Functions.md#Rf-range)
* [F.25: Use a `zstring` or a `not_null<zstring>` to designate a C-style string](S-05-Functions.md#Rf-zstring)
* [F.26: Use a `unique_ptr<T>` to transfer ownership where a pointer is needed](S-05-Functions.md#Rf-unique_ptr)
* [F.27: Use a `shared_ptr<T>` to share ownership](S-05-Functions.md#Rf-shared_ptr)

Value return semantic rules:

* [F.42: Return a `T*` to indicate a position (only)](S-05-Functions.md#Rf-return-ptr)
* [F.43: Never (directly or indirectly) return a pointer or a reference to a local object](S-05-Functions.md#Rf-dangle)
* [F.44: Return a `T&` when copy is undesirable and "returning no object" isn't an option](S-05-Functions.md#Rf-return-ref)
* [F.45: Don't return a `T&&`](S-05-Functions.md#Rf-return-ref-ref)
* [F.46: `int` is the return type for `main()`](S-05-Functions.md#Rf-main)
* [F.47: Return `T&` from assignment operators.](S-05-Functions.md#Rf-assignment-op)
...
* [F.51: Where there is a choice, prefer default arguments over overloading](S-05-Functions.md#Rf-default-args)
...
* [F.54: If you capture `this`, capture all variables explicitly (no default capture)](S-05-Functions.md#Rf-this-capture)
* [F.55: Don't use `va_arg` arguments](S-05-Functions.md#F-varargs)
...
The rule [Keep functions short](S-05-Functions.md#Rf-single) implies "Keep loop bodies short."
Similarly, lambdas used as callback arguments are sometimes non-trivial, yet unlikely to be re-usable.

##### Enforcement

* See [Keep functions short](S-05-Functions.md#Rf-single)
...
* If the function is going to unconditionally move from the argument, take it by `&&`. See [F.18](S-05-Functions.md#Rf-consume).
* If the function is going to keep a copy of the argument, in addition to passing by `const&` (for lvalues),
  add an overload that passes the parameter by `&&` (for rvalues) and in the body `std::move`s it to its destination. Essentially this overloads a "consume"; see [F.18](S-05-Functions.md#Rf-consume).
* In special cases, such as multiple "input + copy" parameters, consider using perfect forwarding. See [F.19](S-05-Functions.md#Rf-forward).
...
* Returning `const T&` from assignments and similar operations (see [F.47](S-05-Functions.md#Rf-assignment-op).)
...
If you have multiple values to return, [use a tuple](S-05-Functions.md#Rf-out-multi) or similar multi-member type.
...
* To reuse an object that carries capacity (e.g., `std::string`, `std::vector`) across multiple calls to the function in an inner loop: [treat it as an in/out parameter and pass by reference](S-05-Functions.md#Rf-out-multi).
...
In such cases, passing the object by reference [`T&`](S-05-Functions.md#Rf-inout) is usually the right technique.
...
A reference is often a superior alternative to a pointer [if there is no need to use `nullptr`](S-05-Functions.md#Rf-ptr-ref) or [if the object referred to should not change](???).

##### Note

Do not return a pointer to something that is not in the caller's scope; see [F.43](S-05-Functions.md#Rf-dangle).
