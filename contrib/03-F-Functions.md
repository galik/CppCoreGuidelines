...
* [F.5: If a function is very small and time-critical, declare it inline](./03-F-Functions.md./03-F-Functions.md#Rf-inline)
...
* [F.17: For "in-out" parameters, pass by reference to non-`const`](./03-F-Functions.md./03-F-Functions.md#Rf-inout)
...
* [F.21: To return multiple "out" values, prefer returning a tuple or struct](./03-F-Functions.md./03-F-Functions.md#Rf-out-multi)
* [F.60: Prefer `T*` over `T&` when "no argument" is a valid option](./03-F-Functions.md./03-F-Functions.md#Rf-ptr-ref)
...
* [F.45: Don't return a `T&&`](./03-F-Functions.md./03-F-Functions.md#Rf-return-ref-ref)
...
Passing a smart pointer transfers or shares ownership and should only be used when ownership semantics are intended (see [R.30](./06-R-Resource management.md./06-R-Resource management.md#Rr-smartptrparam)).
...
See further in [R.30](./06-R-Resource management.md./06-R-Resource management.md#Rr-smartptrparam).
...
**See also**: [when to prefer `T*` and when to prefer `T&`](./03-F-Functions.md./03-F-Functions.md#Rf-ptr-ref).
...
If you have multiple values to return, [use a tuple](./03-F-Functions.md./03-F-Functions.md#Rf-out-multi) or similar multi-member type.
...
* To reuse an object that carries capacity (e.g., `std::string`, `std::vector`) across multiple calls to the function in an inner loop: [treat it as an in/out parameter and pass by reference](./03-F-Functions.md./03-F-Functions.md#Rf-out-multi).
...
In such cases, passing the object by reference [`T&`](./03-F-Functions.md./03-F-Functions.md#Rf-inout) is usually the right technique.
...
A reference is often a superior alternative to a pointer [if there is no need to use `nullptr`](./03-F-Functions.md./03-F-Functions.md#Rf-ptr-ref) or [if the object referred to should not change](???).
...
[Default arguments for virtual functions](./04-C-Classes and Class Hierarchies.md./04-C-Classes and Class Hierarchies.md#Rh-virtual-default-arg)
