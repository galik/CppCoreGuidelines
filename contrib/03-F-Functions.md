...
* [F.1: "Package" meaningful operations as carefully named functions](03-F-Functions%2Emd#Rf-package)
* [F.2: A function should perform a single logical operation](03-F-Functions%2Emd#Rf-logical)
* [F.3: Keep functions short and simple](03-F-Functions%2Emd#Rf-single)
* [F.4: If a function may have to be evaluated at compile time, declare it `constexpr`](03-F-Functions%2Emd#Rf-constexpr)
* [F.5: If a function is very small and time-critical, declare it inline](03-F-Functions%2Emd#Rf-inline)
* [F.6: If your function may not throw, declare it `noexcept`](03-F-Functions%2Emd#Rf-noexcept)
* [F.7: For general use, take `T*` or `T&` arguments rather than smart pointers](03-F-Functions%2Emd#Rf-smart)
* [F.8: Prefer pure functions](03-F-Functions%2Emd#Rf-pure)
* [F.9: Unused parameters should be unnamed](03-F-Functions%2Emd#Rf-unused)

Parameter passing expression rules:

* [F.15: Prefer simple and conventional ways of passing information](03-F-Functions%2Emd#Rf-conventional)
* [F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const`](03-F-Functions%2Emd#Rf-in)
* [F.17: For "in-out" parameters, pass by reference to non-`const`](03-F-Functions%2Emd#Rf-inout)
* [F.18: For "consume" parameters, pass by `X&&` and `std::move` the parameter](03-F-Functions%2Emd#Rf-consume)
* [F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter](03-F-Functions%2Emd#Rf-forward)
* [F.20: For "out" output values, prefer return values to output parameters](03-F-Functions%2Emd#Rf-out)
* [F.21: To return multiple "out" values, prefer returning a tuple or struct](03-F-Functions%2Emd#Rf-out-multi)
* [F.60: Prefer `T*` over `T&` when "no argument" is a valid option](03-F-Functions%2Emd#Rf-ptr-ref)

Parameter passing semantic rules:

* [F.22: Use `T*` or `owner<T*>` or a smart pointer to designate a single object](03-F-Functions%2Emd#Rf-ptr)
* [F.23: Use a `not_null<T>` to indicate "null" is not a valid value](03-F-Functions%2Emd#Rf-nullptr)
* [F.24: Use a `span<T>` or a `span_p<T>` to designate a half-open sequence](03-F-Functions%2Emd#Rf-range)
* [F.25: Use a `zstring` or a `not_null<zstring>` to designate a C-style string](03-F-Functions%2Emd#Rf-zstring)
* [F.26: Use a `unique_ptr<T>` to transfer ownership where a pointer is needed](03-F-Functions%2Emd#Rf-unique_ptr)
* [F.27: Use a `shared_ptr<T>` to share ownership](03-F-Functions%2Emd#Rf-shared_ptr)

Value return semantic rules:

* [F.42: Return a `T*` to indicate a position (only)](03-F-Functions%2Emd#Rf-return-ptr)
* [F.43: Never (directly or indirectly) return a pointer or a reference to a local object](03-F-Functions%2Emd#Rf-dangle)
* [F.44: Return a `T&` when copy is undesirable and "returning no object" isn't an option](03-F-Functions%2Emd#Rf-return-ref)
* [F.45: Don't return a `T&&`](03-F-Functions%2Emd#Rf-return-ref-ref)
* [F.46: `int` is the return type for `main()`](03-F-Functions%2Emd#Rf-main)
* [F.47: Return `T&` from assignment operators.](03-F-Functions%2Emd#Rf-assignment-op)
...
* [F.51: Where there is a choice, prefer default arguments over overloading](03-F-Functions%2Emd#Rf-default-args)
...
* [F.54: If you capture `this`, capture all variables explicitly (no default capture)](03-F-Functions%2Emd#Rf-this-capture)
...
The rule [Keep functions short](03-F-Functions%2Emd#Rf-single) implies "Keep loop bodies short."
Similarly, lambdas used as callback arguments are sometimes non-trivial, yet unlikely to be re-usable.

##### Enforcement

* See [Keep functions short](03-F-Functions%2Emd#Rf-single)
...
Passing a smart pointer transfers or shares ownership and should only be used when ownership semantics are intended (see [R.30](06-R-Resource+management%2Emd#Rr-smartptrparam)).
...
See further in [R.30](06-R-Resource+management%2Emd#Rr-smartptrparam).
...
**See also**: [when to prefer `T*` and when to prefer `T&`](03-F-Functions%2Emd#Rf-ptr-ref).

**See also**: Discussion of [smart pointer use](06-R-Resource+management%2Emd#Rr-summary-smartptrs).
...
* If the function is going to unconditionally move from the argument, take it by `&&`. See [F.18](03-F-Functions%2Emd#Rf-consume).
* If the function is going to keep a copy of the argument, in addition to passing by `const&` (for lvalues),
  add an overload that passes the parameter by `&&` (for rvalues) and in the body `std::move`s it to its destination. Essentially this overloads a "consume"; see [F.18](03-F-Functions%2Emd#Rf-consume).
* In special cases, such as multiple "input + copy" parameters, consider using perfect forwarding. See [F.19](03-F-Functions%2Emd#Rf-forward).
...
* Returning `const T&` from assignments and similar operations (see [F.47](03-F-Functions%2Emd#Rf-assignment-op).)
...
[That could be dangerous](04-C-Classes+and+Class+Hierarchies%2Emd#Rc-move-semantic).
...
If you have multiple values to return, [use a tuple](03-F-Functions%2Emd#Rf-out-multi) or similar multi-member type.
...
* To reuse an object that carries capacity (e.g., `std::string`, `std::vector`) across multiple calls to the function in an inner loop: [treat it as an in/out parameter and pass by reference](03-F-Functions%2Emd#Rf-out-multi).
...
In such cases, passing the object by reference [`T&`](03-F-Functions%2Emd#Rf-inout) is usually the right technique.
...
**See also**: [Support library](20-GSL-Guideline+support+library%2Emd#S-gsl).
...
See also [C.50](04-C-Classes+and+Class+Hierarchies%2Emd#Rc-factory) regarding when to return a `shared_ptr` from a factory.
...
A reference is often a superior alternative to a pointer [if there is no need to use `nullptr`](03-F-Functions%2Emd#Rf-ptr-ref) or [if the object referred to should not change](???).

##### Note

Do not return a pointer to something that is not in the caller's scope; see [F.43](03-F-Functions%2Emd#Rf-dangle).
...
[Default arguments for virtual functions](04-C-Classes+and+Class+Hierarchies%2Emd#Rh-virtual-default-arg)
