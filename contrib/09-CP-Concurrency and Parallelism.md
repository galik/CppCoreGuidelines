...
[(rare) proper uses of `volatile`](09-CP-Concurrency+and+Parallelism.md#Rconc-volatile2)
...
* [CP.25: Prefer `gsl::raii_thread` over `std::thread` unless you plan to `detach()`](09-CP-Concurrency+and+Parallelism.md#Rconc-raii_thread)
* [CP.26: Prefer `gsl::detached_thread` over `std::thread` if you plan to `detach()`](09-CP-Concurrency+and+Parallelism.md#Rconc-detached_thread)
...
* [CP.28: Remember to join scoped `thread`s that are not `detach()`ed](09-CP-Concurrency+and+Parallelism.md#Rconc-join-undetached)
* [CP.30: Do not pass pointers to local variables to non-`raii_thread`s](#Rconc-pass)
* [CP.31: Pass small amounts of data between threads by value, rather than by reference or pointer](09-CP-Concurrency+and+Parallelism.md#Rconc-data-by-value)
...
But that compromises safety and violates the [use RAII](09-CP-Concurrency+and+Parallelism.md#Rconc-raii) rule.
...
* [CP.100: Don't use lock-free programming unless you absolutely have to](09-CP-Concurrency+and+Parallelism.md#Rconc-lockfree)
...
* [CP.111: Use a conventional pattern if you really need double-checked locking](09-CP-Concurrency+and+Parallelism.md#Rconc-double-pattern)
...
Double-checked locking is easy to mess up. If you really need to write your own double-checked locking, in spite of the rules [CP.110: Do not write your own double-checked locking for initialization](09-CP-Concurrency+and+Parallelism.md#Rconc-double) and [CP.100: Don't use lock-free programming unless you absolutely have to](09-CP-Concurrency+and+Parallelism.md#Rconc-lockfree), then do it in a conventional pattern.

The uses of the double-checked locking pattern that are not in violation of [CP.110: Do not write your own double-checked locking for initialization](09-CP-Concurrency+and+Parallelism.md#Rconc-double) arise when a non-thread-safe action is both hard and rare, and there exists a fast thread-safe test that can be used to guarantee that the action is not needed, but cannot be used to guarantee the converse.

##### Example, bad

The use of volatile does not make the first check thread-safe, see also [CP.200: Use `volatile` only to talk to non-C++ memory](09-CP-Concurrency+and+Parallelism.md#Rconc-volatile2)
...
* [CP.200: Use `volatile` only to talk to non-C++ memory](09-CP-Concurrency+and+Parallelism.md#Rconc-volatile2)
